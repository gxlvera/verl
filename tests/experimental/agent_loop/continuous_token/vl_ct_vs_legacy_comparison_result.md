...
`<think>` block at the generation prompt. A content-only assistant message would
then render an *empty* `<think>\n\n</think>` block whose `\n\n` tokenizes
differently from the generation-prompt `<think>\n`, so the suffix diff cannot
align. The harness detects this and makes the mock emit a real
`<think>reasoning</think>` block (the pre-tool thought for tool turns, a short
synthesized thought for the final answer) — exactly what a thinking model emits
at rollout — so the generation prompt stays a clean token prefix. GLM (which is
also a thinking model) is excluded from this path because its empty think block
is already reconciled by a dedicated GLM prompt rewrite.
...
GLM VL      1       3     1     2         0      Single-turn passes; tool cases mismatch because legacy keeps a duplicated assistant-emitted <|observation|> boundary token. CT is correct.
```
...
content-only assistant message would render an empty `<think>\n\n</think>` block
whose `\n\n` tokenizes differently from the generation-prompt `<think>\n` and
breaks the suffix diff. The harness handles this by having the mock emit a real
`<think>reasoning</think>` block — exactly what a thinking model generates at
rollout — after which the CT-vs-legacy comparison exercises the same Qwen ChatML
boundary as Instruct (see below).
...
```text
Model group  vl_singleturnchat  vl_multiturnsingletool  vl_multiturnmultitool
GLM-4.1V     pass               mismatch                mismatch
```

GLM VL uses `tool_parser=glm` and `GLM4VContinuousTokenBuilder` (GLM
observation/user boundary trim + VL processor rendering). The single-image chat
case passes. The tool cases mismatch, and this is the boundary
`GLM4VContinuousTokenBuilder` is designed to correct.
...
```text
response_ids: legacy_len=52 ct_len=51 first_mismatch=36 legacy_value=151338 ct_value=151337
loss_mask:    legacy_len=52 ct_len=51 first_mismatch=36 legacy_value=1      ct_value=0
logprobs:     legacy_len=52 ct_len=51 first_mismatch=36 legacy_value=-0.0118 ct_value=0.0
```

CT (`GLM4VContinuousTokenBuilder._merge_non_assistant_token_ids`) trims the
...
the boundary the next token is the tool-response header rather than the
duplicated `<|observation|>`. The trimmed token carried `loss_mask=1` in legacy
and is correctly dropped by CT. **CT is the correct side.** The image pad-token
spans from the tool responses are identical on both sides.
