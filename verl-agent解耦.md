...
2. 跟望哥已经confirm的点
  ...
  7. 把 DataProto 到 Agent Service 数据协议的转换逻辑写到 RL dataset 里。Agent Service 应该对 DataProto 无感；DataProto 需要先拆成一条一条的样本后才能传给 Agent Service，这个拆分过程也应该写到 RL dataset 里。
