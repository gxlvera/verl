#!/usr/bin/env bash
# Run Qwen3-8B sleep-latency grid with 2 tool calls and 10 training steps.
...
RUN_ID=${RUN_ID:-qwen3_8b_toolcalls2_step10_$(date +%Y%m%d_%H%M%S)}
...
echo "Running Qwen3-8B latency grid: sleeps=${SLEEP_MS_GRID}, min_tool_calls=${GSM8K_MIN_TOOL_CALLS}, total_steps=${TOTAL_STEPS}, run_id=${RUN_ID}"
echo "MODEL_PATH=${MODEL_PATH}"
echo "Common config: TRAIN_BATCH_SIZE=${TRAIN_BATCH_SIZE}, PPO_MINI_BATCH_SIZE=${PPO_MINI_BATCH_SIZE}, PPO_MICRO_BATCH_SIZE_PER_GPU=${PPO_MICRO_BATCH_SIZE_PER_GPU}, ROLLOUT_N=${ROLLOUT_N}, CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}, NGPUS_PER_NODE=${NGPUS_PER_NODE}"

for sleep_ms in ${SLEEP_MS_GRID}; do
    export GSM8K_TOOL_SLEEP_MS="${sleep_ms}"
    export EXPERIMENT_NAME="sglang_metrics_qwen3_8b_sleep${sleep_ms}_toolcalls${GSM8K_MIN_TOOL_CALLS}_bs${TRAIN_BATCH_SIZE}_rollout${ROLLOUT_N}_step${TOTAL_STEPS}_${RUN_ID}"
    ...
    if [[ -s "${SGLANG_METRICS_FILE}" ]]; then
        ...
        python3 examples/agent_loop/plot_sglang_running_requests.py \
            ...
            --title "Qwen3-8B SGLang running requests, sleep=${sleep_ms}ms, min_tool_calls=${GSM8K_MIN_TOOL_CALLS}, bs=${TRAIN_BATCH_SIZE}, rollout_n=${ROLLOUT_N}" \
            >"${LOG_ROOT}/${EXPERIMENT_NAME}_plot_active.log" 2>&1
        active_plot_status=$?

        python3 examples/agent_loop/plot_sglang_step_zoom.py \
            --jsonl "${SGLANG_METRICS_FILE}" \
            --output-png "${LOG_ROOT}/${EXPERIMENT_NAME}_two_main_peaks_zoom_0p1s_fixed_ylim.png" \
            --title "Qwen3-8B SGLang running requests zoom, sleep=${sleep_ms}ms, min_tool_calls=${GSM8K_MIN_TOOL_CALLS}, selected peaks" \
...
echo "Finished Qwen3-8B latency grid: ${RUN_ID}"
