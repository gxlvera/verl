#!/usr/bin/env bash
# BM25 (real content) + simulated SerpAPI latency (search_latencies.csv, mean ~2.7s).
# bs256, ALL await (no fire-and-forget). 7 runs:
#   baseline, full/tail25/tail15 (no spec), full/tail25/tail15 (with spec).
# Spec hit rates: full per-turn 0.30,0.30,0.18,0.15 ; tail uniform 0.18.
# A failed run is logged and skipped so the set still completes.
set -uo pipefail
cd "$(dirname "$0")/../.."
...
echo "$TS" > /tmp/real_bm25_latency_ts.txt
SUMMARY=/home/tiger/verl/logs/real_bm25_latency_${TS}_summary.txt
echo "latency-set ts=$TS  (summary -> $SUMMARY)"
