#!/usr/bin/env bash
# Run HotpotQA Qwen3-8B latency-distribution experiments with and without
# speculative non-thinking tool prefetch.
...
RUN_ID=${RUN_ID:-qwen3_8b_hotpotqa_latency_dist_speculative_$(date +%Y%m%d_%H%M%S)}
...
export HOTPOT_TOOL_LATENCY_SECONDS_LIST=${HOTPOT_TOOL_LATENCY_SECONDS_LIST:-"4.481,1.065,1.849,2.108,2.726,1.272,9.977,1.784,0.879,1.864"}
export HOTPOT_TOOL_SLEEP_MS=0
export GSM8K_TOOL_SLEEP_MS=0
export HOTPOT_TOOL_SLEEP_DIST=""
export GSM8K_TOOL_SLEEP_DIST=""
export HOTPOT_TOOL_SLEEP_SEED=${HOTPOT_TOOL_SLEEP_SEED:-42}
export GSM8K_TOOL_SLEEP_SEED=${GSM8K_TOOL_SLEEP_SEED:-${HOTPOT_TOOL_SLEEP_SEED}}

export PER_TURN_MAX_RESPONSE_LENGTH=${PER_TURN_MAX_RESPONSE_LENGTH:-500}
export AGENT_LOOP_PER_TURN_MAX_RESPONSE_LENGTH=${AGENT_LOOP_PER_TURN_MAX_RESPONSE_LENGTH:-${PER_TURN_MAX_RESPONSE_LENGTH}}
...
export MOCK_BATCH_SIZE=${MOCK_BATCH_SIZE:-128}
...
export DATA_ENABLE_THINKING=${DATA_ENABLE_THINKING:-False}
...
echo "Running Qwen3-8B HotpotQA latency distribution/speculative grid: run_id=${RUN_ID}"
echo "Latency seconds list: ${HOTPOT_TOOL_LATENCY_SECONDS_LIST}"
echo "Config: train_batch=${TRAIN_BATCH_SIZE}, rollout_n=${ROLLOUT_N}, baseline_steps=${BASELINE_STEPS:-10}, partial_steps=${PARTIAL_STEPS:-5}, partial_boundaries=10"

for variant in latency_dist speculative_prefetch; do
    for mode in baseline partial_async; do
        if [[ "${mode}" == "partial_async" ]]; then
            export TOTAL_STEPS=${PARTIAL_STEPS:-5}
            export GSM8K_AGENT_LOOP_MODE=partial_async
            unset GSM8K_PARTIAL_ASYNC
        else
            export TOTAL_STEPS=${BASELINE_STEPS:-10}
            export GSM8K_AGENT_LOOP_MODE=
            unset GSM8K_PARTIAL_ASYNC
        fi

        if [[ "${variant}" == "speculative_prefetch" ]]; then
            export HOTPOT_SPECULATIVE_TOOL_PREFETCH=1
            export HOTPOT_MAIN_ENABLE_THINKING=true
            export DATA_ENABLE_THINKING=true
            export HOTPOT_NON_THINKING_MAX_NEW_TOKENS=${HOTPOT_NON_THINKING_MAX_NEW_TOKENS:-500}
        else
            export HOTPOT_SPECULATIVE_TOOL_PREFETCH=
            export HOTPOT_MAIN_ENABLE_THINKING=
            export DATA_ENABLE_THINKING=False
            export HOTPOT_NON_THINKING_MAX_NEW_TOKENS=
        fi

        export EXPERIMENT_NAME="${variant}_${mode}_hotpotqa_qwen3_8b_latencydist_toolcalls${HOTPOT_MIN_TOOL_CALLS}_bs${TRAIN_BATCH_SIZE}_rollout${ROLLOUT_N}_step${TOTAL_STEPS}_${RUN_ID}"
        ...
        export HOTPOT_SPECULATIVE_JSONL="${LOG_ROOT}/${EXPERIMENT_NAME}_speculative_prefetch.jsonl"
        ...
        if [[ -s "${SGLANG_METRICS_FILE}" ]]; then
            python3 examples/agent_loop/plot_sglang_running_requests.py \
                ...
                --title "HotpotQA ${variant} ${mode} running requests" \
...
echo "Finished Qwen3-8B HotpotQA latency distribution/speculative grid: ${RUN_ID}"
