#!/usr/bin/env bash
# Run sleep=0/300/600ms experiments with 3 tool calls and 20 training steps.
# The prompt order is fixed and rollout decoding is greedy by default.
...
RUN_ID=${RUN_ID:-toolcalls3_step20_deterministic_$(date +%Y%m%d_%H%M%S)}
...
echo "Running deterministic SGLang metrics grid: sleeps=${SLEEP_MS_GRID}, min_tool_calls=${GSM8K_MIN_TOOL_CALLS}, run_id=${RUN_ID}"
echo "Common config: TRAIN_BATCH_SIZE=${TRAIN_BATCH_SIZE}, PPO_MINI_BATCH_SIZE=${PPO_MINI_BATCH_SIZE}, PPO_MICRO_BATCH_SIZE_PER_GPU=${PPO_MICRO_BATCH_SIZE_PER_GPU}, ROLLOUT_N=${ROLLOUT_N}, TOTAL_STEPS=${TOTAL_STEPS}, CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}, NGPUS_PER_NODE=${NGPUS_PER_NODE}"
echo "Determinism config: DATA_SHUFFLE=${DATA_SHUFFLE}, DATA_SEED=${DATA_SEED}, ACTOR_SHUFFLE=${ACTOR_SHUFFLE}, ACTOR_DATA_LOADER_SEED=${ACTOR_DATA_LOADER_SEED}, ROLLOUT_DO_SAMPLE=${ROLLOUT_DO_SAMPLE}, ROLLOUT_TEMPERATURE=${ROLLOUT_TEMPERATURE}"
...
echo "Finished deterministic SGLang metrics grid: ${RUN_ID}"
