#!/usr/bin/env bash
# Run sleep=0ms and sleep=300ms SGLang metrics experiments sequentially, then plot running requests.
...
SLEEP_MS_GRID=${SLEEP_MS_GRID:-"0 300"}
...
echo "Running SGLang metrics grid: sleeps=${SLEEP_MS_GRID}, run_id=${RUN_ID}"
...
for sleep_ms in ${SLEEP_MS_GRID}; do
    ...
    export EXPERIMENT_NAME="sglang_metrics_sleep${sleep_ms}_bs${TRAIN_BATCH_SIZE}_rollout${ROLLOUT_N}_step${TOTAL_STEPS}_${RUN_ID}"
    ...
    if [[ -s "${SGLANG_METRICS_FILE}" ]]; then
        ...
        python3 examples/agent_loop/plot_sglang_running_requests.py \
            ...
            --title "SGLang running requests, sleep=${sleep_ms}ms, bs=${TRAIN_BATCH_SIZE}, rollout_n=${ROLLOUT_N}" \
            >"${LOG_ROOT}/${EXPERIMENT_NAME}_plot.log" 2>&1
        plot_status=$?
        set -e
        if [[ "${plot_status}" != "0" ]]; then
            echo "Plot failed for ${EXPERIMENT_NAME}; see ${LOG_ROOT}/${EXPERIMENT_NAME}_plot.log"
        else
            cat "${LOG_ROOT}/${EXPERIMENT_NAME}_plot.log"
        fi
    else
